        ...
        switch (state) {
            case FRAME_START: {
                ...
                if (frameOpcode > 7) { // control frame (have MSB in opcode set)
                    ...
                    // closeConnection frame : if there is a body, the first two bytes of the body MUST be a 2-byte
                    // unsigned integer (in network byte order) representing a status code
                    if (frameOpcode == 8 && framePayloadLen1 == 1) {
                        protocolViolation(channel, "received closeConnection control frame with payload len 1");
                        return null;
                    }
                } else { // data frame
